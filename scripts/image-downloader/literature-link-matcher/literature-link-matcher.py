# This file finds matching links (URLs) for literature references in the ALL
...
for LemmaURL1 in matching_dict.keys():
    ...
    for LemmaURL2 in literature_dict.keys():
       if LemmaURL1 == LemmaURL2: #Find lemmas that have both urls/links in them, as well as literature sources
           ...
           #Create list of linkdicts from matching_dict (for specific LemmaURL)
           linkdictlist = [linkdict for linkdict in matching_dict[LemmaURL1]]
           #print('linkdictlist = ' + str(linkdictlist))
           ...
           if litlist: #Litlist does not always exist
               for linkdict in linkdictlist:
                   for lit in litlist:
                       if linkdict['LinkLabel'] in lit: # by chaning this to eg linkdict['LinkLabel'][2:-2] you can omit first 2 and last 2 chars from matching linklabel, for looser matching
                          print('{}*****{}*****{}*****{}'.format(LemmaURL1,linkdict['LinkLabel'],lit,linkdict['LinkURL']))
